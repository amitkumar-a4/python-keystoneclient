...
from workloadmanager.openstack.common import lockutils
...
synchronized = lockutils.synchronized_with_prefix('workloadmanager-')
...
def find_config(config_path):
    ...
    :raises: `workloadmanager.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(FLAGS.state_path, "etc", "workloadmanager", config_path),
        os.path.join(FLAGS.state_path, "etc", config_path),
        os.path.join(FLAGS.state_path, config_path),
        "/etc/workloadmanager/%s" % config_path,
    ]
...
def execute(*cmd, **kwargs):
    ...
    corresponding filter to etc/workloadmanager/rootwrap.d !
    ...
    if run_as_root:
        ...
        if FLAGS.rootwrap_config is None or FLAGS.root_helper != 'sudo':
            LOG.deprecated(_('The root_helper option (which lets you specify '
                             'a root wrapper different from workloadmanager-rootwrap, '
                             'and defaults to using sudo) is now deprecated. '
                             'You should use the rootwrap_config option '
                             'instead.'))

        if (FLAGS.rootwrap_config is not None):
            cmd = ['sudo', '/opt/stack/workloadmanager/bin/workloadmanager-rootwrap',
...
def workloadmanagerdir():
    import workloadmanager
    return os.path.abspath(workloadmanager.__file__).split('workloadmanager/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'workloadmanager.api.ec2.cloud:' \
     workloadmanager.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See workloadmanager.openstack.common.notifier.api.notify_decorator)
...
def generate_mac_address():
    ...
    #             properly: 0xfa.
    #             Discussion: https://bugs.launchpad.net/workloadmanager/+bug/921838
    mac = [0xfa, 0x16, 0x3e,
