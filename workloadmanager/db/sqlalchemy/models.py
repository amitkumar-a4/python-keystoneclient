...
"""
SQLAlchemy models for workloadmanager data.
"""
...
from workloadmanager.db.sqlalchemy.session import get_session
...
class WorkloadManagerBase(object):
    """Base class for WorkloadManager Models."""
...
class Service(BASE, WorkloadManagerBase):
    ...
    availability_zone = Column(String(255), default='workloadmanager')

class VaultServices(BASE, WorkloadManagerBase):
...
class WorkloadManagerNode(BASE, WorkloadManagerBase):
    """Represents a running workloadmanager service on a host."""

    __tablename__ = 'workloadmanager_nodes'
    id = Column(Integer, primary_key=True)
    service_id = Column(Integer, ForeignKey('services.id'), nullable=True)


                           
class BackupJob(BASE, WorkloadManagerBase):
...
class BackupJobVMs(BASE, WorkloadManagerBase):
...
class ScheduledJobs(BASE, WorkloadManagerBase):
...
class BackupJobRuns(BASE, WorkloadManagerBase):
...
class BackupJobRunVMs(BASE, WorkloadManagerBase):
    """Represents vms of a backup job"""
...
class VMRecentBackupJobRun(BASE, WorkloadManagerBase):
...
class BackupJobRunVMResources(BASE, WorkloadManagerBase):
...
class VMResourceBackups(BASE, WorkloadManagerBase):
...
class VMResourceBackupMetadata(BASE, WorkloadManagerBase):
...
def register_models():
    ...
    Called from workloadmanager.db.sqlalchemy.__init__ as part of loading the driver,
