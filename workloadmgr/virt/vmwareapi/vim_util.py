...
def get_object_properties(vim, collector, mobj, type, properties):
    ...
    obj_content = vim.RetrievePropertiesEx(
        usecoll, specSet=[property_filter_spec], options=options)
...
def get_dynamic_property(vim, mobj, type, property_name):
    ...
    try:
        property_dict = get_dynamic_properties(
            vim, mobj, type, [property_name])
        return property_dict.get(property_name)
    except Exception as ex:
        msg = _("Cannot find properties %(property)s for VM %(vm)s. ") % {
            'property': str(property_name), 'vm': mobj.value}
...
def get_dynamic_properties(vim, mobj, type, property_names):
    ...
    try:
        ...
        msg = _("Cannot find properties %(properties)s for VM %(vm)s. ") % {
            'properties': str(property_names), 'vm': mobj.value}
        raise error_util.VimAttributeError(msg, ex)
