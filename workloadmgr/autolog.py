...
"""
    This file has been *heavily* modified to remove the use of global
    variables, implement a logging class instead of relying on sys.stdout,
    remove the function log decorator, remove the module log decorator,
    allow color changing on any log call, allow indentation level changing
    on any log call, and PEP-8 formatting.
...
def log_method(logger, method_name=None, log_args=True,
    ...
    """
        use this for class or instance methods, it formats
        with the object out front.
    """
    def _real_log_method(method):
        def _wrapper(*args, **kwargs):
            if log_args:
                ...
                arg_str = ''
            message_enter = "{method_color}{method_name}{message_color} ENTER {normal_color}\
                ({arg_str}) {file_name} {line_num}".format(**{
                'method_color': BROWN,
            ...
            message_exit = "{method_color}{method_name}{message_color} EXIT {normal_color}\
                (Return Value(s):{ret_val}) {file_name} {line_num}".format(**{
                'method_color': BROWN,
                'method_name': "{0}".format(method_name or method.__name__),
                'message_color': BLUE,
                'normal_color': NORMAL,
