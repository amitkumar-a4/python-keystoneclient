...
def find_alive_nodes(defaultnode, SSHPort, Username,
                     Password, dbuser, dbpassword,
                     DBPort, addlnodes=None):
    ...
    try:
        ...
        pssh_exec_command(nodes, int(SSHPort), Username,
                          Password, cmd)
        nodelist = nodes
        ...
        error_msg = _("Failed to authenticate on host %s with \
                    host username/password") % (str(addlnodes))
        ...
        for host in nodes:
            try:
                ...
                pssh_exec_command(
                    [host], int(SSHPort), Username, Password,
                    'mongo --eval ' +
...
def pssh_exec_command(hosts, port, user, password, command, sudo=False):
    try:
        ...
        client = ParallelSSHClient(hosts, user=user, password=password,
                                   port=int(port), timeout=int(timeout))
...
def get_databases(hosts, port, username, password,
    ...
    for host in output:
        if output[host]['exit_code']:
            LOG.warning(
                _(cmd + "on %s cannot be executed. Error %s. Error Msg: %s") %
                (host, str(output[host]['exit_code']), output[host]['stdout']))
            continue
    ...
    if not clusterinfo['ok'] and "not master" in clusterinfo['errmsg']:
                ...
                if replica['stateStr'] == 'PRIMARY':
                    ...
                    if primary_host != host:  # break the recursion
                        clusterinfo = get_databases(
                            [primary_host], port, username, password,
                            primary_dbport, dbuser, dbpassword)
                    else:
                        msg = _("Failed to execute 'mongo --eval' \
                              successfully. Error %s") % \
                              (clusterinfo['errmsg'])
                        LOG.error(msg)
                        raise exception.ErrorOccurred(msg)
                    break
        ...
        msg = _("Failed to execute 'mongo --eval' \
              successfully. Error %s") % (clusterinfo['errmsg'])
        LOG.error(msg)
        raise exception.ErrorOccurred(msg)
...
def main(argv):
    try:
        ...
        alivenodes = find_alive_nodes(defaultnode, port, username, password,
                                      dbuser, dbpassword, dbport, addlnodes)

        clusterinfo = {}
        clusterinfo['databases'] = get_databases(alivenodes, port, username,
                                                 password, dbport, dbuser,
                                                 dbpassword)
        ...
        LOG.exception(ex)
        usage = _("Usage: mongodbnodes.py --config-file "
                  "/etc/workloadmgr/workloadmgr.conf --defaultnode mongodb1 "
                  "--port 22 --username ubuntu --password password "
                  "--dbport <mongos/mongodport> "
                  "--dbuser <mongo admin> "
                  "--dbpassword <password> "
                  "--addlnodes 'mongodb1;mongodb2;mongodb3' "
                  "--outfile /tmp/mongodbnodes.txt --errfile "
                  "/tmp/mongodbnodes_error.txt")
        LOG.info(usage)
