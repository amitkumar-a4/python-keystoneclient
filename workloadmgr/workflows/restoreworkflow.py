...
def get_vms(cntx, restore_id):
    ...
    for snapshot_vm in snapshot_vms:
        ...
        elif instance_options.get('include', True) is True:
            snapshots_vms_to_be_restored.append(snapshot_vm)
    ...
    sequence = 0
    while ((len(vms_with_power_sequence) + len(vms_without_power_sequence)) <
           len(snapshot_vms)):
        for snapshot_vm in snapshot_vms:
            vm = {'vm_id': snapshot_vm.vm_id,
                  'vm_name': snapshot_vm.vm_name,
...
class RestoreWorkflow(object):
    ...
    def initflow(self, pre_poweron=None, post_poweron=None):
        ...
        # restore networks
        self._flow.add(vmtasks.RestoreVMNetworks(
            "RestoreVMNetworks", provides='restored_net_resources'))

        # restore security_groups
        self._flow.add(vmtasks.RestoreSecurityGroups(
            "RestoreSecurityGroups", provides='restored_security_groups'))
