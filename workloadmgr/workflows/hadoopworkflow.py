...
def connect_server(host, port, user, password):
    try:
        ...
        LOG.error(_('There was an error connecting to hadoop namenode. \
                    Error %s. Try again...'), str(e))
    return client


def gethadoopcfg(connection):
    stdin, stdout, stderr = \
        connection.exec_command("hadoop/bin/hadoop dfsadmin -report")
    dfsout = stdout.read()
        ...
        for addr in instance.addresses:
            ...
            for _if in ifs:
                if _if['addr'] in ips:
                    ...
                    inst_hyper = instance.__dict__['OS-EXT-SRV-ATTR:\
                                                   hypervisor_hostname']
                    for hypervisor in hypervisors:
                        if hypervisor.hypervisor_hostname == inst_hyper:
                            hypervisor_hostname = inst_hyper
                            hypervisor_type = hypervisor.hypervisor_type
                            break

                    inst_pstate = instance.__dict__['OS-EXT-STS:power_state']
                    utils.append_unique(
                        vms,
                        {'vm_id': instance.id,
...
class HadoopWorkflow(workflow.Workflow):
    ...
    # : usename/password - username and password to
    #  authenticate to the namenode
    #
    def initflow(self, composite=False):
        cntx = amqp.RpcContext.from_dict(self._store['context'])

        self._store['instances'] = get_hadoop_nodes(
            cntx, self._store['Namenode'],
            int(self._store['NamenodeSSHPort']),
            self._store['Username'], self._store['Password'])
    ...
    def topology(self):
        ...
        hadoopstatus = []
        hadoopstatus.append(['Capacity',
                             hadoopcfg['hconfig']['Configured Capacity']])
        hadoopstatus.append(['Remaining',
                             hadoopcfg['hconfig']['DFS Remaining']])
    ...
    def execute(self):
        ...
        engines.run(
            self._flow, engine_conf='parallel',
...
#context = context.RequestContext(
               "1c4c73dfb2724ef7a4e659999db40be5", #admin user id
               #c.client.projectid,
               #is_admin = True,
...
#result = engines.load(hwf._flow, engine_conf='parallel',
