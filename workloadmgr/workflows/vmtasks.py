...
class WorklodMgrDB(base.Base):

    def __init__(self, host=None, db_driver=None):
        super(WorklodMgrDB, self).__init__(db_driver)
                
class PauseVM(task.Task):

    def execute(self, context_dict, instance_id):
        # Pause the VM
        import pdb; pdb.set_trace()
        db = WorklodMgrDB().db
        context = amqp.RpcContext.from_dict(context_dict)
...
def UnorderedPauseVMs(instances):
    ...
    for index,item in enumerate(instances):
        flow.add(PauseVM("PauseVM_" + item, rebind=dict(instance_id = "instanceid_" + str(index))))
    return flow
...
def UnorderedSnapshotVMs(instances):
    ...
    for index,item in enumerate(instances):
        flow.add(SnapshotVM("SnapshotVM_" + item, rebind=dict(instance_id = "instanceid_" + str(index))))
...
def UnorderedResumeVMs(instances):
    ...
    for index,item in enumerate(instances):
        flow.add(ResumeVM("ResumeVM_" + item, rebind=dict(instance_id = "instanceid_" + str(index))))
...
def UnorderedUploadSnapshots(instances):
    flow = uf.Flow("resumevmsuf")
    for index,item in enumerate(instances):
        flow.add(UploadSnapshot("UploadSnapshot_" + item, rebind=dict(instance_id = "instanceid_" + str(index))))
...
def UnorderedBlockCommit(instances):
    flow = uf.Flow("resumevmsuf")
    for index,item in enumerate(instances):
        flow.add(BlockCommit("BlockCommit_" + item, rebind=dict(instance_id = "instanceid_" + str(index))))
