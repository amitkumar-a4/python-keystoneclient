...
class CompositeWorkflow(workflow.Workflow):
    ...
    def initflow(self):
        ...
        for workflowid, workflow in workflows.iteritems():
            ...
            # Snapshot Metadata operates on list of snapshots,
            # so we only need to have one snapshot vmnetworks
            # and snapshot flavor
            break
        ...
        super(CompositeWorkflow, self).initflow(
    ...
    def execute(self):
        ...
        vmtasks.CreateVMSnapshotDBEntries(
            self._store['context'], self._store['instances'],
