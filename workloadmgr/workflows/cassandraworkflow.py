...
def _exec_command(connection, command):
    stdin, stdout, stderr = \
        connection.exec_command('bash -c "' + command + '"', timeout=120)
    err_msg = stderr.read()
    if err_msg != '':
        stdin, stdout, stderr = connection.exec_command(command, timeout=120)
        err_msg = stderr.read()
        if err_msg != '':
            raise Exception(_("Error connecting to Cassandra \
                              Service on %s - %s") %
...
def connect_server(host, port, user, password):
    try:
        ...
        LOG.error(_('There was an error connecting to cassandra node. \
                    Error %s. Try again...'), str(ex))
        raise ex
    return client
...
class SnapshotCassandraNode(task.Task):
    ...
    def execute(self, context, snapshot, CassandraNode,
        ...
        return self.execute_with_log(context, snapshot, CassandraNode,
                                     SSHPort, Username, Password)
    ...
    def execute_with_log(self, context, snapshot, CassandraNode,
                         SSHPort, Username, Password):
        try:
            ...
            db.snapshot_update(
                cntx, snapshot['id'],
                {'progress_msg': 'Invoking Cassandra Snapshot on ' +
                                 CassandraNode})
            self.client = connect_server(CassandraNode, int(SSHPort),
                                         Username, Password)
            LOG.info(_('SnapshotNode:' + CassandraNode))
            stdin, stdout, stderr = \
                _exec_command(self.client, "nodetool snapshot")
            out = stdout.read(),
            LOG.info(_("nodetool snapshot output:" + str(out)))
            self.client.close()
        except Exception as ex:
            LOG.exception(ex)
            raise exception.ErrorOccurred(_("Unable to run nodetool snapshot \
                                            command on %s") % CassandraNode)
        return
    ...
    def revert_with_log(self, *args, **kwargs):
        ...
        try:
            if not isinstance(kwargs['result'], misc.Failure):
                ...
                self.client = connect_server(kwargs['CassandraNode'],
                                             int(kwargs['SSHPort']),
                                             kwargs['Username'],
                                             kwargs['Password'])
                stdin, stdout, stderr = \
                    _exec_command(self.client, "nodetool clearsnapshot")
                out = stdout.read(),
                LOG.info(_("revert Snapshotnode nodetool clearsnapshot \
                           output:" + str(out)))
        except Exception as ex:
            LOG.exception(ex)
        finally:
    ...
    def execute_with_log(self, context, snapshot, CassandraNode,
        ...
        try:
            ...
            db.snapshot_update(cntx, snapshot['id'],
                               {'progress_msg': 'Clearing Cassandra \
                                Snapshot on ' + CassandraNode})
            self.client = connect_server(CassandraNode, int(SSHPort),
                                         Username, Password)
            ...
            stdin, stdout, stderr = \
                _exec_command(self.client, "nodetool clearsnapshot")
            out = stdout.read(),
            LOG.info(_("ClearSnapshot nodetool clearsnapshot output:" +
                       str(out)))
            self.client.close()
        except Exception as ex:
            LOG.exception(ex)
            raise exception.ErrorOccurred(_("Unable to run \
                nodetool clearsnapshot command on %s") % CassandraNode)
        finally:
            if self.client:
                self.client.close()
...
def UnorderedSnapshotCassandraNode(instances):
    ...
    for index, item in enumerate(instances):
        flow.add(SnapshotCassandraNode(
            "SnapshotCassadraNode_" + item['vm_id'],
            rebind=("context", "snapshot",
                    'CassandraNodeHostName_'+item['vm_id'],
                    "SSHPort", "Username", "Password")))
    return flow
...
def UnorderedClearCassandraSnapshot(instances):
    ...
    for index, item in enumerate(instances):
        flow.add(ClearCassandraSnapshot(
            "ClearCassandraSnapshot_" + item['vm_id'],
            rebind=("context",
                    "snapshot",
                    'CassandraNodeHostName_'+item['vm_id'],
                    "SSHPort", "Username", "Password")))
...
def get_cassandra_nodes(store, findpartitiontype='False'):
    ...
    try:
        ...
        return clusterinfo['preferrednodes'],\
            clusterinfo['allnodes'], clusterinfo
...
def get_cassandra_instances(store, findpartitiontype='False'):
    ...
    try:
        cassandra_nodes, allnodes, clusterinfo = \
            get_cassandra_nodes(store, findpartitiontype=findpartitiontype)
        ...
        for node in cassandra_nodes:
            if 'MacAddresses' in node:
                for macaddress in node['MacAddresses']:
                    ...
                    root_partition_type[macaddress.lower()] = \
                        node.get('root_partition_type', 'lvm')
        ...
        # call nova interface-list <instanceid> to build the list of
        # instances ids if node names are host names then lookup
        # the VMid based on the
        for instance in instances:
            # The following logic helps for VMware VMs.
            # For OpenStack instances, look at the instance interfaces.
            for addr in json.loads(instance.metadata['networks']):
                # IP Addresses this is our vm
                if addr['macAddress'].lower() in interfaces:
                    hypervisor_hostname = None
                    hypervisor_type = "VMware vCenter Server"
                    clustername = "Unknown"

                    if 'cluster' in instance.metadata and \
                       instance.metadata['cluster']:
                    ...
                    utils.append_unique(
                        ...
                        {'vm_id': instance.id,
                         ...
                         'root_partition_type':
                             root_partition_type[addr['macAddress'].lower()],
                         'vm_power_state':
                             instance.__dict__['OS-EXT-STS:power_state'],
        ...
        if len(vms) == 0:
            LOG.info(_("No VMs are discovered in tvault inventory.\
                       Please run discover and try again"))
            raise Exception(_("No instances are discovered in \
                       tvault inventory. Please run discover and try again"))
...
class CassandraWorkflow(workflow.Workflow):
    ...
    def initflow(self, composite=False):
        try:
            self._store['instances'], cassandra_nodes,\
                allnodes, clusterinfo = \
            ...
            for index, item in enumerate(self._store['instances']):
                ...
                self._store['CassandraNodeName_' + item['vm_id']] = \
                    item['vm_name']
                self._store['CassandraNodeHostName_' + item['vm_id']] = \
                    item['hostname']
            ...
            snapshotvms.add(
                UnorderedSnapshotCassandraNode(self._store['instances']))
            ...
            # instance ids
            snapshotvms.add(
                vmtasks.UnorderedPauseVMs(self._store['instances']))

            # This is again unorder snapshot of VMs. This flow is
            # implemented in common tasks library
            snapshotvms.add(
                vmtasks.UnorderedSnapshotVMs(self._store['instances']))

            # This is an unordered pausing of VMs.
            snapshotvms.add(
                vmtasks.UnorderedUnPauseVMs(self._store['instances']))

            # enable profiling to the level before the flow started
            snapshotvms.add(
                UnorderedClearCassandraSnapshot(self._store['instances']))

            super(CassandraWorkflow, self).initflow(
                snapshotvms, composite=composite)
    ...
    def topology(self, cassnodes=None, clusterinfo=None):
        try:
            ...
            if cassnodes is None or clusterinfo is None:
                cassnodes, allnodes, clusterinfo = \
                    get_cassandra_nodes(self._store, findpartitiontype='False')
                cassnodes = allnodes
            ...
            for n in cassnodes:
                ...
                if not n['Rack'] in \
                   dcs["datacenters"][n['Data Center']]["racks"]:
                    dcs["datacenters"][n['Data Center']]["racks"][n['Rack']] =\
                        {'name': n['Rack'], "nodes": {}, "input": []}

                # if not n['Address'] in dcs["datacenters"]
                # [n['Data Center']]["racks"][n['Rack']]["nodes"]:
                n['name'] = n['Address']
                ...
                dcs["datacenters"][n['Data Center']]["racks"]\
                    [n['Rack']]["nodes"][n['Address']] = \
                    {'name': n['Address'], "node": n}
    ...
    def details(self):
        ...
        def recurseflow(item):
            if isinstance(item, task.Task):
                taskdetails = {'name': item._name.split("_")[0],
                               'type': 'Task'}
                taskdetails['input'] = []
    ...
    def discover(self):
        try:
            instances, cassnodes, allnodes, clusterinfo = \
                get_cassandra_instances(self._store, findpartitiontype='False')
            for instance in instances:
    ...
    def execute(self):
        ...
        vmtasks.CreateVMSnapshotDBEntries(self._store['context'],
...
def update_cassandra_yaml(mountpath, nodeip, ips):
    ...
    configpath = ""
    if os.path.exists(
       os.path.join(mountpath, 'usr/share/dse/resources/cassandra/conf/')):
...
def update_cassandra_topology_yaml(mountpath, address, broadcast):
    ...
    yamlbakpath = os.path.join(mountpath, configpath,
...
def update_cassandra_topology_properties(mountpath, addresses):
    ...
    topopath = os.path.join(mountpath, configpath,
                            'cassandra-topology.properties')
    topobakpath = os.path.join(mountpath, configpath,
        ...
        with open(envpath, 'w') as fout:
            for line in f:
                if "java.rmi.server.hostname" in line:
                    line = 'JVM_OPTS="$JVM_OPTS -Djava.rmi.server.\
                           hostname=' + hostname + '"\n'
                    line += 'JVM_OPTS="$JVM_OPTS -Dcassandra.\
                            load_ring_state=false"\n'
                fout.write(line)
                ...
                if line.startswith("source"):
                    ...
                    if line.split()[1].strip().rstrip() == "lo":
                        ...
                        while not line.strip().startswith("auto") and \
                              not line.strip().startswith("source"):  # nopep8
                            newinf.write(line)
                            line = f.readline()
                    else:
                        stanza = create_interface_stanza(
...
class CassandraRestoreNode(task.Task):
    ...
    def execute_with_log(self, context, instance, restore,
        ...
        cntx = amqp.RpcContext.from_dict(context)
        process, mountpath = \
            vmtasks_vcloud.mount_instance_root_device(cntx, instance, restore)
        try:
...
def LinearCassandraRestoreNodes(workflow):
    ...
    for index, item in enumerate(workflow._store['instances']):
        ...
        flow.add(CassandraRestoreNode("CassandraRestoreNode_" + item['vm_id'],
                                      rebind=rebind_dict))
...
class CassandraRestore(restoreworkflow.RestoreWorkflow):
    ...
    def initflow(self):
        options = pickle.loads(
            self._store['restore']['pickle'].encode('ascii', 'ignore'))
        if 'restore_options' in options and options['restore_options'] != {}:
            ...
            if 'Nodename' in restore_options:
                for vmname, hostname in \
                      restore_options['Nodename'].iteritems():
                    if hostname == "":
                        self._store['hostname_' + str(vmname)] = \
                            vmname + "_restored"
                    else:
                ...
                for index, item in enumerate(self._store['instances']):
                    self._store['hostname_' + str(index)] = \
                        item['vm_name'] + "_restored"
