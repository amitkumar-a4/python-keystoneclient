...
def snapshot_vm_networks(cntx, db, instances, snapshot):
    try:
        ...
        # refresh the token. token may have been invalidated during
        # long running tasks during upload and post snapshot processing
        user_id = cntx.user
...
def get_vm_nics(cntx, db, instance, restore, restored_net_resources):
    ...
    db.restore_update(
        cntx, restore['id'],
        {'progress_msg': 'Restoring network interfaces for Instance ' +
                         instance['vm_id']})
    restored_nics = []
    snapshot_vm_resources = db.snapshot_vm_resources_get(
        cntx, instance['vm_id'], restore['snapshot_id'])
