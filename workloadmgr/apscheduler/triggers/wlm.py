...
# from workloadmgr.apscheduler.util import convert_to_datetime,
# timedelta_seconds
...
class WorkloadMgrTrigger(object):
    def __init__(self, jobschedule):
        ...
        if 'start_date' in jobschedule and \
           jobschedule['start_date'].strip(" ").lower() != "now":
            self.start_date = convert_to_datetime(
                parse(jobschedule['start_date'] + " " +
                      jobschedule['start_time']))

        if 'end_date' in jobschedule and \
           jobschedule['end_date'].strip(" ").lower() != "no end":
            ...
            if jobschedule['interval'].find("hr"):
                if int(jobschedule['interval'].strip(" ").split("hr")[0]) >= 1:
                    self.interval = timedelta(
                        hours=int(jobschedule['interval'].strip(" ").split(
                           "hr")[0]))
                else:
