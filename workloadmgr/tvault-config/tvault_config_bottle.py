...
aaa = Cork(
    'conf',
    email_sender='info@triliodata.com',
...
def login():
    ...
    aaa.login(
        username,
        password,
        success_redirect='/change_password',
        fail_redirect='/login')
...
def change_password():
    """Change password"""
    aaa.current_user.update(
        pwd=post_get('newpassword'),
        email_addr="info@triliodata.com")
    bottle.redirect("/home")
            ...
            if user.name == 'triliovault':
                if keystone.version == 'v3':
                    keystone.users.update(
                        user, password=config_inputs['newpassword'])
                else:
                    keystone.users.update_password(
                        user, config_inputs['newpassword'])
...
def send_upstart_logs(filename):
    return static_file(filename, root='/var/log/upstart',
...
def send_wlm_logs(filename):
    return static_file(filename, root='/var/log/workloadmgr',
                       mimetype='text/plain', download=True)
...
def send_tvault_gui_logs(filename):
    return static_file(filename, root='/var/log/tvault-gui',
                       mimetype='text/plain', download=True)
...
def send_nova_logs(filename):
    return static_file(filename, root='/var/log/nova',
                       mimetype='text/plain', download=True)
...
def send_neutron_logs(filename):
    return static_file(filename, root='/var/log/neutron',
                       mimetype='text/plain', download=True)
...
def send_keystone_logs(filename):
    return static_file(filename, root='/var/log/keystone',
                       mimetype='text/plain', download=True)
...
def send_tvault_logs():
    try:
        ...
        logtarfilename = '/tmp/tvaultlogs/tvaultlogs_' + \
            datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S") + '.tar'
...
def send_tvaultlogs_all():
    try:
        ...
        logtarfilename = '/tmp/tvaultlogs_all/tvaultlogs_' + \
            datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S") + '.tar'
        logtar = tarfile.open(name=logtarfilename, mode='w:gz')
        if os.path.exists('/var/log/workloadmgr'):
            logtar.add('/var/log/workloadmgr')
...
def _authenticate_with_swift(config_data):
    if config_data['configuration_type'] == 'vmware' or config_data['configuration_type'] == 'openstack':
        if config_data['swift_auth_url'] and len(
                config_data['swift_auth_url']) > 0:
...
def _authenticate_with_keystone():
    ...
    try:
        ...
        raise Exception(
            "KeystoneError:Unable to connect to keystone Admin URL " +
            e.message)

    if keystone.version == 'v3':
        keystone_endpoints = keystone.endpoints.list(
            service=keystone.services.find(
                type='identity').id,
    ...
    if keystone_admin_url != config_data['keystone_admin_url']:
        raise Exception(
            "KeystoneError:Please enter correct keystone admin url ")
    ...
    if 'endpoint_type' not in config_data:
        raise Exception(
            "KeystoneError:Please enter correct keystone public/internal url ")
...
def _register_service():
    ...
    if config_data['nodetype'] != 'controller':
        ...
        if wlm_user is None:
            raise Exception(
                "Trilio Vault Appliance controller node may not have been configured. Cannot find 'triliovault' user")
    ...
    for service in services:
        if service.type == 'workloads':
            for endpoint in endpoints:
                if endpoint.service_id == service.id and endpoint.region == config_data[
                        'region_name']:
    ...
    if config_data['enable_tls'] == 'on':
        ...
        wlm_url = 'http://' + \
            config_data['tvault_primary_node'] + ':8780' + '/v1/$(tenant_id)s'
...
def configure_mysql():
    if config_data['nodetype'] == 'controller':
        ...
        subprocess.call(command, shell=False)
        stmt = 'GRANT ALL PRIVILEGES ON *.* TO ' + '\'' + 'root' + '\'' + '@' + '\'' + \
            '%' + '\'' + ' identified by ' + '\'' + TVAULT_SERVICE_PASSWORD + '\'' + ';'
        command = [
            'sudo',
            'mysql',
            '-uroot',
...
def configure_keystone():
    ...
    if config_data['nodetype'] == 'controller' and config_data['configuration_type'] == 'vmware':
        ...
        Config.set(
            'ldap',
            'use_dumb_member',
            config_data['ldap_use_dumb_member'])
        Config.set(
            'ldap',
            'user_allow_create',
            config_data['ldap_user_allow_create'])
        Config.set(
            'ldap',
            'user_allow_update',
            config_data['ldap_user_allow_update'])
        Config.set(
            'ldap',
            'user_allow_delete',
            config_data['ldap_user_allow_delete'])
        Config.set(
            'ldap',
            'tenant_allow_create',
            config_data['ldap_tenant_allow_create'])
        Config.set(
            'ldap',
            'tenant_allow_update',
            config_data['ldap_tenant_allow_update'])
        Config.set(
            'ldap',
            'tenant_allow_delete',
            config_data['ldap_tenant_allow_delete'])
        Config.set(
            'ldap',
            'role_allow_create',
            config_data['ldap_role_allow_create'])
        Config.set(
            'ldap',
            'role_allow_update',
            config_data['ldap_role_allow_update'])
        Config.set(
            'ldap',
            'role_allow_delete',
            config_data['ldap_role_allow_delete'])
        Config.set(
            'ldap',
            'user_objectclass',
            config_data['ldap_user_objectclass'])
        Config.set(
            'ldap',
            'user_name_attribute',
            config_data['ldap_user_name_attribute'])
        ...
        try:
                ...
                if service.type == 'identity' or service.type == 'keystone':
                    for endpoint in endpoints:
                        if endpoint.service_id == service.id and endpoint.region == config_data[
                            ...
                            keystone.services.delete(service.id)
            # create service and endpoint
            identity_service = keystone.services.create(
                'keystone', 'identity', 'Trilio Vault Identity Service')
            public_url = 'http://' + \
                config_data['tvault_primary_node'] + ':5000' + '/v2.0'
            admin_url = 'http://' + \
                config_data['tvault_primary_node'] + ':35357' + '/v2.0'
            ...
            try:
                keystone.roles.add_user_role(
                    user=user_id, role=admin_role_id, tenant=admin_tenant_id)
...
def configure_nova():
    if config_data['nodetype'] == 'controller' and config_data['configuration_type'] == 'vmware':
                ...
                if service.type == 'compute' or service.type == 'computev3':
                    for endpoint in endpoints:
                        if endpoint.service_id == service.id and endpoint.region == config_data[
                            ...
                            keystone.services.delete(service.id)
            # create service and endpoint
            compute_service_v2 = keystone.services.create(
                'nova', 'compute', 'Trilio Vault Compute Service')
            public_url = 'http://' + \
                config_data['tvault_primary_node'] + \
                ':8774' + '/v2/$(tenant_id)s'
            keystone.endpoints.create(
                config_data['region_name'],
                compute_service_v2.id,
                public_url,
                public_url,
                public_url)

            compute_service_v3 = keystone.services.create(
                'trilioVaultCS-V3', 'computev3', 'Trilio Vault Compute Service')
            public_url = 'http://' + \
                config_data['tvault_primary_node'] + \
                ':8774' + '/v3/$(tenant_id)s'
            keystone.endpoints.create(
                config_data['region_name'],
                compute_service_v3.id,
                public_url,
                public_url,
                public_url)
        except Exception as exception:
        ...
        command = [
            ...
            'sh',
            '-c',
            "echo manual > /etc/init/nova-conductor.override"]
        subprocess.call(command, shell=False)
        command = [
            'sudo',
            'sh',
...
def configure_neutron():
    if config_data['nodetype'] == 'controller' and config_data['configuration_type'] == 'vmware':
        ...
        command = [
            'sudo',
            'rm',
            "/etc/init/neutron-plugin-openvswitch-agent.override"]
        subprocess.call(command, shell=False)
                ...
                if service.type == 'network':
                    for endpoint in endpoints:
                        if endpoint.service_id == service.id and endpoint.region == config_data[
                            ...
                            keystone.services.delete(service.id)
            # create service and endpoint
            network_service = keystone.services.create(
                'neutron', 'network', 'Trilio Vault Network Service')
            public_url = 'http://' + \
                config_data['tvault_primary_node'] + ':9696' + '/'
            keystone.endpoints.create(
                config_data['region_name'],
                network_service.id,
                public_url,
                ...
                if service.type == 'image':
                    for endpoint in endpoints:
                        if endpoint.service_id == service.id and endpoint.region == config_data[
                            ...
                            keystone.services.delete(service.id)
            # create service and endpoint
            image_service = keystone.services.create(
                'glance', 'image', 'Trilio Vault Image Service')
            public_url = 'http://' + \
...
def service_action(service_display_name, action):
    ...
    bottle.request.environ['beaker.session']['error_message'] = ''
    return dict(
        error_message=bottle.request.environ['beaker.session']['error_message'])
...
def troubleshooting_vmware():
    ...
    try:
        ...
        vcenter_username = config_data.get(
            'vcenter_username', 'not_configured')
        vcenter_password = config_data.get(
            'vcenter_password', 'not_configured')
        if config_status == 'not_configured':
        ...
        for vm_name in values['reset_cbt_vms'].split(","):
            ...
            if len(vm_refs) != 1:
                ...
                continue
            vm_ref = vm_refs[0]
            rootsnapshot = session._call_method(
                vim_util,
            ...
            if not snapshot_ref:
                raise Exception(
                    "Failed to create temporary snapshot for Virtul Machine '" +
                    vm_name +
                    "'")
    ...
    try:
        ...
        if len(config_data['name_server']):
            ...
            if len(config_data['domain_search_order']):
                new_file.write(
                    'search ' +
                    config_data['domain_search_order'] +
                    '\n')
            ...
            if len(mounts) == 0 or mounts[0].get(mountpath, None) != nfsshare:
                ...
                try:
                    temp_file_name = os.path.join(
                        mountpath, str(uuid.uuid4()) + '_test.txt')
                    ...
                    subprocess.check_call(command, shell=False)
                    raise Exception(
                        "Failed to verify R/W permissions of the NFS export: " + nfsshare)

        tmpfs_path = os.path.join(
            config_data['vault_data_directory_old'], "tmpfs")
        ...
        if os.path.exists(config_data['vault_data_directory']):
            for d in os.listdir(config_data['vault_data_directory']):
                if os.path.ismount(os.path.join(
                        config_data['vault_data_directory'], d)):
                    cleanup_mount(
                        os.path.join(
                            config_data['vault_data_directory'], d))

        nfsoptions = config_data['storage_nfs_options']
        for idx, nfsshare in enumerate(
                str.split(config_data['storage_nfs_export'], ',')):
            if nfsshare == '' or (
                    config_data['swift_auth_version'] == 'KEYSTONE' or config_data['swift_auth_version'] == 'TEMPAUTH'):
                continue
            replace_line(
                '/etc/hosts.allow',
                'rpcbind : ',
            ...
            mountpath = os.path.join(
                config_data['vault_data_directory'], base64encode)
    ...
    for i in range(0, 1):
        try:
            ...
            if "KeystoneError:" in exception.message:
                bottle.request.environ['beaker.session']['error_message'] = " %(exception)s" % {
                    'exception': exception, }
                return bottle.HTTPResponse(status=500, body="Error")
            else:
                pass
...
def configure_api():
    ...
    try:
        ...
        replace_line(
            '/opt/tvault-gui/config/tvault-gui.yml',
            '    ip: ',
            '    ip: ' +
            config_data['keystone_host'])
        replace_line('/opt/tvault-gui/config/tvault-gui.yml', '    port: ',
                     '    port: ' + str(config_data['keystone_public_port']))
    ...
    try:
        if config_data['nodetype'] == 'controller':
            ...
            command = [
                ...
                'sh',
                '-c',
                "echo manual > /etc/init/wlm-scheduler.override"]
            # shell=FALSE for sudo to work.
            subprocess.call(command, shell=False)
    except Exception as exception:
        bottle.request.environ['beaker.session']['error_message'] = "Error: %(exception)s" % {
...
def configure_service():
    ...
    try:
        ...
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'glance_production_host = ',
            'glance_production_host = ' +
            config_data['glance_production_host'])
        replace_line('/etc/workloadmgr/workloadmgr.conf', 'glance_production_port = ',
                     'glance_production_port = ' + str(config_data['glance_production_port']))
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'glance_production_api_servers = ',
        ...
        replace_line(
            ...
            'neutron_admin_auth_url = ',
            'neutron_admin_auth_url = ' +
            config_data['neutron_admin_auth_url'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'neutron_production_url = ',
            'neutron_production_url = ' +
            config_data['neutron_production_url'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'neutron_admin_username = ',
            'neutron_admin_username = ' +
            config_data['neutron_admin_username'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'neutron_admin_password = ',
            'neutron_admin_password = ' +
            config_data['neutron_admin_password'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'neutron_admin_tenant_name = ',
            'neutron_admin_tenant_name = ' +
            config_data['neutron_admin_tenant_name'])

        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'nova_admin_auth_url = ',
            'nova_admin_auth_url = ' +
            config_data['nova_admin_auth_url'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
        ...
        replace_line(
            ...
            'nova_admin_username = ',
            'nova_admin_username = ' +
            config_data['nova_admin_username'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'nova_admin_password = ',
            'nova_admin_password = ' +
            config_data['nova_admin_password'])

        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'cinder_production_endpoint_template = ',
            'cinder_production_endpoint_template = ' +
            config_data['cinder_production_endpoint_template'])
        ...
        replace_line(
            ...
            'vault_data_directory = ' +
            config_data['vault_data_directory'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
        ...
        replace_line(
            ...
            'vault_storage_nfs_options = ' +
            config_data['storage_nfs_options'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'cloud_unique_id = ',
            'cloud_unique_id = ' +
            config_data['cloud_unique_id'])
        ...
        replace_line(
            ...
            'sql_connection = ',
            'sql_connection = ' +
            config_data['sql_connection'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'rabbit_host = ',
            'rabbit_host = ' +
            config_data['rabbit_host'])
        replace_line(
            '/etc/workloadmgr/workloadmgr.conf',
            'rabbit_password = ',
            'rabbit_password = ' +
            config_data['rabbit_password'])
        ...
        replace_line(
            ...
            'auth_host = ' +
            config_data['keystone_host'])
        replace_line('/etc/workloadmgr/api-paste.ini', 'auth_port = ',
                     'auth_port = ' + str(config_data['keystone_admin_port']))
        replace_line(
            '/etc/workloadmgr/api-paste.ini',
            'auth_protocol = ',
            'auth_protocol = ' +
            config_data['keystone_admin_protocol'])
        replace_line(
            '/etc/workloadmgr/api-paste.ini',
            'admin_user = ',
            'admin_user = ' +
    ...
    try:
        ...
        for key, value in config_data.iteritems():
            ...
            if config_data['nodetype'] == 'controller':
                engine = create_engine(config_data['sql_connection'])
                name_found = False
                for persisted_setting in engine.execute(
                        select([models.Settings.__table__])):
...
def configure_vmware():
    ...
    try:
        ...
        config_data['keystone_admin_url'] = "http://" + \
            config_data['tvault_primary_node'] + ":35357/v2.0"
        config_data['keystone_public_url'] = "http://" + \
            config_data['tvault_primary_node'] + ":5000/v2.0"
...
def configure_openstack():
    ...
    try:
        ...
        if config_data['nodetype'] == 'controller':
            ...
            sql_connection = 'mysql://root:' + TVAULT_SERVICE_PASSWORD + '@' + \
                config_data['floating_ipaddress'] + '/workloadmgr?charset=utf8'
            engine = create_engine(sql_connection)
            rows = engine.execute(select([models.Settings.__table__]).where(
                models.Settings.__table__.columns.project_id == 'Configurator'))
            ...
            for row in rows:
                ...
                if items['name'] == 'tvault_ipaddress':
                    if items['value'] == config_data['tvault_ipaddress']:
                        raise Exception(
                            "Additional node IP address cannot be Controller node, Enter Controller node Ip address")
        ...
        Config.read('/etc/workloadmgr/workloadmgr.conf')
        old_password = Config.get('keystone_authtoken', 'admin_password')
        config_data['workloadmgr_user_password'] = old_password
...
def home():
    ...
    msgs = dict(
        error_message=bottle.request.environ['beaker.session']['error_message'],
        success_message=bottle.request.environ['beaker.session']['success_message'])
    bottle.request.environ['beaker.session']['error_message'] = ''
    bottle.request.environ['beaker.session']['success_message'] = ''
    return msgs
...
def main_http():
    ...
    bottle.TEMPLATE_PATH.insert(
        0, '/opt/stack/workloadmgr/workloadmgr/tvault-config/views')
...
def main():
    ...
    try:
        try:
            ovfEnv = subprocess.Popen(
                "echo `vmtoolsd --cmd \"info-get guestinfo.ovfenv\"`",
                shell=True,
