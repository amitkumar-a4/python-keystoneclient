...
"""
   ...
   parallel-ssh uses asychronous network requests - there is
   *no* multi-threading or multi-processing used.

   This is a *requirement* for commands on many (hundreds/thousands/hundreds
   of thousands) of hosts which would grind a system to a halt simply by
   having so many processes/threads all wanting to execute if done with
   multi-threading/processing.

   The `libev event loop library <http://software.schmorp.de/pkg/libev.html>`_
   is utilised on nix systems. Windows is not supported.

   See :mod:`pssh.ParallelSSHClient` and :mod:`pssh.SSHClient`
...
class SSHClient(object):
    ...
    def __init__(self, host,
        ...
        :param num_retries: (Optional) Number of retries for connection
        ...
        if self.proxy_host and self.proxy_port:
            logger.debug("Proxy configured for destination host %s - \
                         Proxy host: %s:%s",
    ...
    def _connect(self, client, host, port, sock=None, retries=1):
        ...
        try:
            ...
            """
            logger.info("%s - %s - retry %s/%s" % (str(ex.args[1]), host,
                        retries, self.num_retries))
            ...
            """
            raise ConnectionErrorException("%s for host '%s:%s' - \
                                           retry %s/%s" %
    ...
    def exec_command(self, command, sudo=False, user=None, **kwargs):
        ...
        if sudo or user:
            if stdin.channel.closed is False:
                # If stdin is still open then sudo is asking us for a password
    ...
    def mkdir(self, sftp, directory):
        ...
        Catches and logs at error level remote IOErrors on
        creating directory."""
        try:
    ...
    def copy_file(self, local_file, remote_file):
        ...
        Copy is done natively using SFTP/SCP version 2 protocol, no \
        scp command is used or required.
...
class ParallelSSHClient(object):
    """
       Uses :mod:`pssh.SSHClient`, performs tasks over SSH on multiple hosts
        in parallel.

       Connections to hosts are established in parallel when ``run_command``
       is called, therefor any connection and/or authentication exceptions
       will happen on the call to ``run_command`` and need to be caught.
    """

    def __init__(self, hosts,
                 user=None, password=None, port=None, pkey=None,
                 forward_ssh_agent=True, num_retries=DEFAULT_RETRIES,
                 timeout=None, pool_size=10, proxy_host=None, proxy_port=22):
        """
        :param hosts: Hosts to connect to
        :type hosts: list(str)
        :param user: (Optional) User to login as. Defaults to logged
        in user or user from ~/.ssh/config or /etc/ssh/ssh_config if set
        :type user: str
        :param password: (Optional) Password to use for login. Defaults to \
        no password
        :type password: str
        :param port: (Optional) Port number to use for SSH connection.
        Defaults to None which uses SSH default
        :type port: int
        :param pkey: (Optional) Client's private key to be used to connect with
        :type pkey: :mod:`paramiko.PKey`
        :param num_retries: (Optional) Number of retries for connection
        ...
        whichever is lower. Pool size will be *equal to* number of hosts
        if number of hosts is lower than the pool size specified as that
        would only increase overhead with no benefits.
        :type pool_size: int
        :param proxy_host: (Optional) SSH host to tunnel connection through
        ...
        **Example with specified private key**
        ...
        .. note ::
          ...
          **Connection persistence**
          ...
          object's life. To close them, just `del` or reuse the
          object reference.
          ...
          :netstat:
          ``tcp        0      0 127.0.0.1:53054   127.0.0.1:22  ESTABLISHED``
    ...
    def run_command(self, *args, **kwargs):
        ...
        then return immediately. Any connection and/or authentication
        exceptions will be raised here and need catching.
...
future releases - use self.run_command instead", DeprecationWarning)
    ...
    def get_stdout(self, greenlet, return_buffers=False):
        ...
        **Deprecated** - use self.get_output() instead.
        ...
        :rtype: Dictionary containing ``{host: {'exit_code': exit code}}``
        entry
        for example ``{'myhost1': {'exit_code': 0}}``
        :rtype: With ``return_buffers=True``:
                      ``{'myhost1': {'exit_code': 0,
                         'channel' : None or SSH channel of command
                                     if command is still executing,
                         'stdout' : <iterable>,
                         'stderr' : <iterable>,}}``
        """
        warnings.warn("This method is being deprecated and will be removed in \
                      future releases - use self.get_output instead",
                      DeprecationWarning)
        gevent.sleep(.2)
        ...
        if channel.exit_status_ready():
            ...
            logger.debug("Command still executing on get_stdout call - not \
                         closing channel and returning None as exit code.")
            # If channel is not closed we cannot get full stdout/stderr
            # so must return buffers
            return_buffers = True
        ...
        gevent.sleep(.2)
        return {host: {'exit_code': channel.recv_exit_status()
                       if channel.exit_status_ready() else None,
