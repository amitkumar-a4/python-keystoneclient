...
"""Unit tests for `workloadmgr.wsgi`."""
...
class TestLoaderNothingExists(test.TestCase):
    ...
    def test_config_not_found(self):
        self.assertRaises(
            workloadmgr.exception.ConfigNotFound,
            workloadmgr.wsgi.Loader,
        )
...
document_root = /tmp
    ...
    def setUp(self):
        ...
        self.config.seek(0)
        self.config.flush()
        self.loader = workloadmgr.wsgi.Loader(self.config.name)
    ...
    def test_app_not_found(self):
        self.assertRaises(
            workloadmgr.exception.PasteAppNotFound,
...
class TestWSGIServer(test.TestCase):
    ...
    def test_no_app(self):
        server = workloadmgr.wsgi.Server("test_app", None)
    ...
    def test_app(self):
        ...
        server = workloadmgr.wsgi.Server("test_app", hello_world)
        server.start()
...
class ExceptionTest(test.TestCase):
    ...
    def _do_test_exception_safety_reflected_in_faults(self, expose):
        class ExceptionWithSafety(exception.WorkloadMgrException):
            safe = expose
        ...
        self.assertIn('{"workloadmgrFault', resp.body)
    ...
    def test_non_workloadmgr_notfound_exception_mapping(self):
    ...
    def test_non_workloadmgr_exception_mapping(self):
        class ExceptionWithCode(Exception):
            code = 417
