...
def _get_tenant_context(user_id, tenant_id):
    try:
        ...
        tenantcontext = context.RequestContext(
...
def novaclient(context, production, admin=False, extensions=None):
    if admin:
        ...
        c.client.auth_token = context.auth_token or \
            '%s:%s' % (context.user_id, context.project_id)
        c.client.management_url = url
    return c
...
class API(base.Base):
    ...
    def get_hypervisors(self, context):
        hypervisors = novaclient(
            context, self._production, True).hypervisors.list()
        return hypervisors
    ...
    def create_server(self, context, name, image, flavor,
        ...
        try:
            ...
            time.sleep(15)
            # TODO(gbasava): Creation is asynchronous. Wait and check for
            # the status
            ...
            item = client.servers.create(
                ...
                config_drive=config_drive, **kwargs)
            time.sleep(15)
            # TODO(gbasava): Creation is asynchronous.
            # Wait and check for the status
    ...
    def get_server_by_id(self, context, id, admin=False, search_opts=None):
        ...
        while retries:
            try:
                if search_opts is None:
                    servers = self._get_servers(context, search_opts,
                                                admin=admin)
                    ...
                    for opt, val in six.iteritems(search_opts):
                        if val:
                            qparams[opt] = val
                    if qparams:
                        new_qparams = sorted(qparams.items(),
                                             key=lambda x: x[0])
                    ...
                    server = client.servers._get(
                        "/servers/%s%s" % (id, query_string), "server")
                    return server
            except nova_exception.Unauthorized:
                retries -= 1
                admin = True
    ...
    def create_flavor(self, context, name, memory, vcpus,
        ...
        try:
            client = novaclient(context, self._production, admin=True)
            return client.flavors.create(name, memory, vcpus,
                                         root_gb, flavorid="auto",
    ...
    def map_snapshot_files(self, context, server, params):
        ...
        try:
            ...
            client = novaclient(context, self._production,
                                extensions=extensions)
            return client.contego.map_snapshot_files(server=server,
                                                     params=params)
        except nova_exception.Unauthorized:
            client = novaclient(context, self._production,
            ...
            LOG.exception(ex)
            msg = 'Unable to map snapshot; Please check contego logs for \
                   more details'
    ...
    def vast_prepare(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to prepare instance for snapshot operation; \
    ...
    def vast_instance(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to snapshot; Please check contego logs for \
                   more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def vast_get_info(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to get instace info; Please check contego logs \
                   for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def vast_data_transfer(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to upload snapshot; Please check contego logs \
                   for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def copy_backup_image_to_volume(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to copy backup image to volume; Please check \
                   contego logs for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def vast_async_task_status(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to get snapshot upload task status; Please check \
                   contego logs for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def vast_finalize(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to complete snapshot operation; Please check \
                   contego logs for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def vast_reset(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to reset workload; Please check contego logs for \
                   more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def testbubble_attach_volume(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to attach volume to test restore; Please check \
                   contego logs for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
    ...
    def testbubble_reboot_instance(self, context, server, params):
        ...
        try:
            ...
            LOG.exception(ex)
            msg = 'Unable to reboot test restore instance; Please check \
                   contego logs for more details'
            raise exception.ErrorOccurred(msg)
            # TODO(gbasava): Handle the exception
