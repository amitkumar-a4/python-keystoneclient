...
def get_instances():
    project_ids = {}
    project_ids['project_id'] = get_config_value(NOVA_SECTION, 'vault_swift_tenant')
    
    url = get_config_value(NOVA_SECTION, 'nova_production_endpoint_template', project_ids)
    cs = nova.novaclient2(get_config_value(NOVA_SECTION, 'nova_admin_auth_url'), get_config_value(NOVA_SECTION, 'nova_admin_username'), get_config_value(NOVA_SECTION, 'nova_admin_password'), get_config_value(NOVA_SECTION, 'vault_swift_tenant'), url)
...
def get_token():
    ...
    values = {"auth": {"tenantName": get_config_value(NOVA_SECTION, 'vault_swift_tenant'), "passwordCredentials": {"username": get_config_value(NOVA_SECTION, 'nova_admin_username'), "password": get_config_value(NOVA_SECTION, 'nova_admin_password')}}}
    headers = {'Content-Type': 'application/json'}
    data = json.loads(execute_post(url, values, headers))
    return data['access']['token']['id'], data['access']['token']['tenant']['id']

def create_workload(inst):
    workload_payload = {'name': 'New Workload', 'workload_type_id': 'f82ce76f-17fe-438b-aa37-7a023058e50d', 'description': 'New Workload', 'source_platform': 'openstack', 'instances': inst, 'jobschedule': {'end_date': 'No End', 'start_time': scheduler_time, 'interval': '24hr', 'enabled': True, 'retain_value': 30, 'retain_type': '0', 'start_date': scheduler_date}, 'metadata': {}}
