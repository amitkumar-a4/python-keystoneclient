...
date_obj = datetime.strptime(scheduler_date+' '+scheduler_time,
                             '%m/%d/%Y %I:%M%p')
...
def get_instances(tenant_name):
    ...
    url = get_config_value(NOVA_SECTION,
                           'nova_production_endpoint_template',
                           project_ids)
    cs = nova.novaclient2(get_config_value(NOVA_SECTION,
                                           'nova_admin_auth_url'),
                          get_config_value(NOVA_SECTION,
                                           'nova_admin_username'),
                          get_config_value(NOVA_SECTION,
                                           'nova_admin_password'),
                          tenant_name, url)
    ...
    try:
        ...
        if tenant_name is None:
            values = {"auth":
                      {"passwordCredentials":
                       {
                        "username": get_config_value(NOVA_SECTION,
                                                     'nova_admin_username'),
                        "password": get_config_value(NOVA_SECTION,
                                                     'nova_admin_password')
                       }}}
        else:
            values = {"auth":
                      {"tenantName": tenant_name, "passwordCredentials":
                       {
                        "username": get_config_value(NOVA_SECTION,
                                                     'nova_admin_username'),
                        "password": get_config_value(NOVA_SECTION,
...
def create_workload(inst, instance_name, tenant_name):
    ...
    dobj = date_obj + timedelta(
        minutes=random.randrange(0, window*60, interval))
    ...
    try:
        ...
        url = 'http://'+get_config_value(DEFAULT_SECTION, 'rabbit_host') +\
              ':8780/v1/' + project_id + '/workloads'
        headers = {'Content-Type': 'application/json', 'X-Auth-Token': token}
        execute_post(url, {'workload': workload_payload}, headers)
...
def main():
    try:
        engine = create_engine(get_config_value(DEFAULT_SECTION,
                                                'sql_connection'),
                               echo=False)
        auto_wlm_vms = DEFAULT_VMS_PER_WORKLOAD
        config_auto_wlm_vms = get_config_value(DEFAULT_SECTION,
                                               'auto_workload_vm_number')
        ...
        config_scheduler_interval = get_config_value(
            DEFAULT_SECTION,
            'automated_workload_scheduler_interval')
        if config_scheduler_interval is not None:
            interval = int(config_scheduler_interval)

        config_window = get_config_value(DEFAULT_SECTION,
                                         'automated_workload_backup_window')
        ...
        for tenant in tenants:
            ...
            for instance in instances:
                ...
                if instance.status == 'ACTIVE' and auto_workload_found is True:
                    d_inst = {}
                    for row in engine.execute(
                        select([models.WorkloadVMs.__table__]).where(
                            models.WorkloadVMs.__table__.
                            columns.vm_id == instance.id)):
                        items = dict(row.items())
                        if items['vm_id'] == instance.id and\
                           items['status'] == 'available':
                            auto_workload_found = False
                            break

                    if auto_workload_found is True:
                        d_inst['instance-id'] = instance.id
                        inst.append(d_inst)

                    if len(inst) == auto_wlm_vms:
                        create_workload(inst, instance.name+" et.al",
                                        tenant['name'])
                        inst = []
