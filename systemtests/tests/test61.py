...
class test61(WorkloadMgrSystemTest):
    ...
    def run(self, *args, **kwargs):
        ...
        self._testshell.cs.workloads.snapshot(self.composite.id, name="Snapshot1", description="First snapshot of the workload")
        ...
        for i in range(0,5):
           # perform snapshot operation
           self._testshell.cs.workloads.snapshot(self.composite.id, name="Snapshot-" + str(i), description="Snapshot of worklaod" + self.composite.id)

           snapshots = []
           for s in self._testshell.cs.snapshots.list():
               if s.workload_id == self.composite.id and s.name == "Snapshot-"+str(i):
                   snapshots.append(s)
