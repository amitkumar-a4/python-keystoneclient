...
class test1(WorkloadMgrSystemTest):
    ...
    def run(self, *args, **kwargs):
        ...
        self.metadata = {'key1':'string', 'key2':'boolean', 'key3':'password', 'key4':'boolean'}
        self.name = "WorkloadType1"
        self.description = "Test Workload type"
        self.workloadtype = self._testshell.cs.workloadtypes.create(metadata=metadata, name=name, description=description)
        status = self.workloadtype.status

        while 1:
           self.workloadtype = self._testshell.cs.workloadtypes.get(self.workloadtype.id)
           status = self.workloadtype.status
