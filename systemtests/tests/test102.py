...
class test102(WorkloadMgrSystemTest):
    ...
    def run(self, *args, **kwargs):
        ...
        self.workload = self._testshell.cs.workloads.create("VMsWorkload", "Workload with 5 VMs", self.serialtype.id, instances, jobschedule, {})
        ...
        jobschedule = {'start_date': 'Now', 'end_date':"No End", 'start_time': "12:00AM", 'interval':'2 hr', 'snapshots_to_keep':10}
        self._testshell.cs.workloads.update(self.workload.id, None, None, self.workload.workload_type_id, None, jobschedule, None)
        self.workload = self._testshell.cs.workloads.get(self.workload.id)
        
        for key, value in jobschedule.iteritems():
            if self.workload.jobschedule[key] != value:
               raise Exception ("Workload update failed. Expected value for key '" + key +"' is '" + value + "'. Got '" + self.workload.jobschedule[key] + "'.")
